...
public class MemberRepository {
    ...
    public Boolean userExistsByExternalClientId(String externalClientId) {
        ...
        CriteriaQuery<ApiUser> criteria = cb.createQuery(ApiUser.class);
        Root<ApiUser> member = criteria.from(ApiUser.class);
        // Swap criteria statements if you would like to try out type-safe criteria queries, a new
        // feature in JPA 2.0
        // criteria.select(member).where(cb.equal(member.get(Member_.name), email));
        criteria.select(member).where(cb.equal(member.get("externalClientId"), externalClientId));
        return !em.createQuery(criteria).getResultList().isEmpty();
    }
    public ApiUser getUserByExternalCLientId(String externalClientId) {
        CriteriaBuilder cb = em.getCriteriaBuilder();
        CriteriaQuery<ApiUser> criteria = cb.createQuery(ApiUser.class);
        Root<ApiUser> member = criteria.from(ApiUser.class);
        // Swap criteria statements if you would like to try out type-safe criteria queries, a new
    ...
    public List<User> findAllOrderedByName2() {
        CriteriaBuilder cb = em.getCriteriaBuilder();
